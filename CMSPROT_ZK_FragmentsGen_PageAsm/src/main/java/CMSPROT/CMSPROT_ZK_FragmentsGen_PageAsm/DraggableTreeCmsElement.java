...
public class DraggableTreeCmsElement extends DraggableTreeElement {
	...
	public DraggableTreeCmsElement(DraggableTreeElement parent, Map<String,String> elementDataMap) {
		...
		this.elementDataMap=new HashMap<String, String>(elementDataMap);	//WARNING we need a copy here, not just the reference! (because we'll reuse the same pipeHashMap instance for every passage)
